...
void mmu_init(void) { 
    ...
    // initialize the MMU hardware state: invalidate all caches and TLBs
    // to a known-good state before we configure the MMU.
    mmu_reset();

    // trivial: RMW the xp bit in control reg 1.
    // leave mmu disabled.
    // B4-9 (armv6.annot.pdf) / arm1176 3-44: XP=1 enables ARMv6 page
    // table format (subpages disabled), required for VMSAv6 and XN bit.
    cp15_ctrl_reg1_t c = cp15_ctrl_reg1_rd();
    c.XP_pt = 1;
    cp15_ctrl_reg1_wr(c);
...
uint32_t domain_access_ctrl_get(void) {
    //inline_asm("MCR p15, 0, r0, c3, c0, 0");
    //PREFETCH_FLUSH(0);
    // return staff_domain_access_ctrl_get();
    // B4-42: Domain Access Control Register = c3, c0, 0 (MRC to read).
    uint32_t r;
...
void domain_access_ctrl_set(uint32_t r) {                                                                                               
      ...
      // B2-24: after CP15 write, must flush BTB; Rd SBZ
      uint32_t z = 0;
      asm volatile("mcr p15, 0, %0, c7, c5, 6" :: "r"(z));//FLUSH_BTB
      // B2-7.5, B2-24: prefetch flush to wait for BTB flush to complete
      asm volatile("mcr p15, 0, %0, c7, c5, 4" :: "r"(z));//PREFETCH_FLUSH
...
cp15_ctrl_reg1_t cp15_ctrl_reg1_rd(void) {
    ...
    // B4-40: Control Register 1 = c1, c0, 0 (MRC to read).
    cp15_ctrl_reg1_t r;
